...
class BehaviourProfileStore {
    ...
    fun apply(event: BehaviourEvent) {
        ...
        profiles.compute(
            ...
            current.copy(
                ...
                joins = current.joins +
                    if (event.action == BehaviourAction.JOIN) 1L else 0L,

                quits = current.quits +
                    if (event.action == BehaviourAction.QUIT) 1L else 0L,

                movements = current.movements +
                    if (event.action == BehaviourAction.MOVE) 1L else 0L,

                blocksBroken = current.blocksBroken +
                    if (event.action == BehaviourAction.BLOCK_BREAK) 1L else 0L,

                blocksPlaced = current.blocksPlaced +
                    if (event.action == BehaviourAction.BLOCK_PLACE) 1L else 0L,

                combatActions = current.combatActions +
                    if (event.action == BehaviourAction.COMBAT) 1L else 0L,
    ...
    fun get(playerId: UUID): PlayerBehaviourProfile? {
